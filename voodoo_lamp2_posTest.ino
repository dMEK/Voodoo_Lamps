...
void loop() {
  for(int i = 0; i <5 ; i++) {
        ...
        //potVal[i]= map(potVal[i], 0, 1023, 0, 180);     // scale it to use it with the servo (value between 0 and 180)
        lampServo[i].write(10);                  // sets the servo position according to the scaled value
        delay(500);        // waits for the servo to get there
  } 
  for(int i = 0; i <5 ; i++) {
          //potVal[i] = analogRead(controlPot[i]);            // reads the value of the potentiometer (value between 0 and 1023)
          //potVal[i]= map(potVal[i], 0, 1023, 0, 180);     // scale it to use it with the servo (value between 0 and 180)
          lampServo[i].write(90);                  // sets the servo position according to the scaled value
          delay(500);        // waits for the servo to get there
    }  
  for(int i = 0; i <5 ; i++) {
          //potVal[i] = analogRead(controlPot[i]);            // reads the value of the potentiometer (value between 0 and 1023)
          //potVal[i]= map(potVal[i], 0, 1023, 0, 180);     // scale it to use it with the servo (value between 0 and 180)
          lampServo[i].write(170);                  // sets the servo position according to the scaled value
          delay(500);        // waits for the servo to get there
    }    
}
