...
void loop() {
  for(int i = 0; i <5 ; i++) {
        potVal[i] = analogRead(controlPot[0]);            // reads the value of the potentiometer (value between 0 and 1023)
        potVal[i]= map(potVal[i], 0, 1023, 0, 180);     // scale it to use it with the servo (value between 0 and 180)
        lampServo[i].write(potVal[i]);                  // sets the servo position according to the scaled value
